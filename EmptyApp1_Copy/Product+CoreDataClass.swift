...
public class Product: NSManagedObject
{
    static func addProduct(id:Int,name: String, productDescription: String, productRating: Decimal, companyId: Int, quantity:Int,logo:Data?)
    {
        ...
        let pt = Product(context: context)
        pt.id = Int64(id)
        pt.name = name
    ...
    static func viewAllProducts() -> [Product]? {
        let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
    ...
    static func SearchProduct(id: Int) -> Product? {
        let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<Product> = Product.fetchRequest()
    ...
    static func searchProductWithRating(productRating: Decimal) -> Product? {
        let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
        let fetchRequest: NSFetchRequest<Product> = Product.fetchRequest()
