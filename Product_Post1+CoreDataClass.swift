//
//  Product_Post1+CoreDataClass.swift
//  EmptyApp1_Copy
//
...
@objc(Product_Post1)
public class Product_Post1: NSManagedObject
{
    static func addProduct_Post(id : Int , productTypeId : Int , companyid :Int , productId : Int,postedDate:Date ,price : Double, desc : String)
        {
            ...
            let pt = Product_Post1(context: context)
            pt.id = Int64(id)
        ...
        static func viewAllPP() -> [Product_Post1]? {
            let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
            let request: NSFetchRequest<Product_Post1> = Product_Post1.fetchRequest()
